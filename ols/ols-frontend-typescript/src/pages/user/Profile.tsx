...
function Profile() {
  ...
  const handleBuyNow = async (orderId: number) => {
    try {
      ...
      await updateCourseStatus(orderId, newOrderStatus);
      alert("Course Buy successful!");
  ...
  const handleEnrollNow = async (courseId: number) => {
    try {
      if (!profile) return;
  ...
  const handleEditEmail = async (userId: number) => {
    if (!newUserEmail) {
      alert("Please enter a new email.");
      return;
    }
    try {
      await updateEmail(userId, newUserEmail);
      alert("Email Updated");
      window.location.reload();
    } catch (error) {
  ...
  return (
    <div className=" vw-100 container py-5">
      <Navbar />
            ...
            {isEditing ? (
              <>
                ...
                <button
                  ...
                  onClick={() => handleEditEmail(profile.userId)}
                >
                  ✅
                </button>
                <button
        ...
        <div className="row">
          {profile.orders.length > 0 ? (
            profile.orders.map((order, index) => {
              const bgColor =
                order.orderStatus === "PAID" ? "bg-success" : "bg-danger";
              return (
                <div key={index} className="col-md-4">
                  <div
                    className={`card shadow-sm p-3 mb-3 text-black ${bgColor}`}
                  >
                    <div className="card-body">
                      ...
                      <p>
                        <strong>Course:</strong> {order.course.courseName}
                      </p>
                      {order.orderStatus === "PENDING" ? (
                        <button
                          className="btn btn-success"
                          onClick={() => handleBuyNow(order.orderId)}
                        >
                          Buy Now
                        </button>
                      ) : (
                        !profile.courses.some(
                          (c) => c.courseId === order.course.courseId
                        ) && (
                          <button
                            className="btn btn-warning"
                            onClick={() =>
                              handleEnrollNow(order.course.courseId)
                            }
                          >
                            Enroll Now
                          </button>
